...
- For schedule_updates that change or remove an existing item, copy its title exactly from the CARE CASE Schedule context. Keep clock times in the time field, not in the title. A user changing the time of an existing recurring reminder requires action "update" on that same item; never add a second reminder for the time change.
